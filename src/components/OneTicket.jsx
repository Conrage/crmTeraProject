...
export default function OneTicket(props) {
  ...
  const [ticketID, setID] = useState(`${props.ticket._id}`);
  ...
  const deleteTicket = () => {
    axios.delete(
      `https://teracrm.herokuapp.com/crm-tera/ticket/delete/${ticketID}`
    );
    closeModal();
  };
  ...
  return (
    <>
      ...
      >
        ...
        <td>{assigned}</td>
        <td className="urgent">{priority}</td>
        <td className="inprogress">{status}</td>
      </tr>
